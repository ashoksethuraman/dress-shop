...
import {authenticate, requireAdmin, validate, setAuthCookies, clearAuthCookies, generateCsrfToken} from "../middleware";
import {type UpdateProfileBody, type SetAdminClaimBody, type SignupBody, type LoginBody} from "../types";
import {validateUpdateProfile, validateSetAdminClaim, validateSignup, validateLogin, validateBulkStatusUpdate} from "../validators";
...
usersRouter.post("/signup", validate(validateSignup), async (req: Request, res: Response) => {
  ...
  const existingMobile = await db.collection("users")
    .where("mobileNumber", "==", normalizedMobile).limit(1).get();
  if (!existingMobile.empty) {
    res.status(409).json({error: "This mobile number is already registered.", field: "mobileNumber"});
    return;
  }
  ...
  await db.doc(`users/${uid}`).set({
    id: uid, username: username.trim(), email: normalizedEmail, passwordHash,
    age, gender, mobileNumber: mobileNumber.trim(), address: address?.trim() ?? null,
...
usersRouter.post("/login", validate(validateLogin), async (req: Request, res: Response) => {
...
usersRouter.get("/csrf-token", (_req: Request, res: Response) => {
  const csrfToken = generateCsrfToken();
  res.cookie("XSRF-TOKEN", csrfToken, {
    httpOnly: false, secure: IS_PROD,
    sameSite: IS_PROD ? "none" : "lax",
    maxAge: 3600 * 1000, path: "/",
  });
  res.json({csrfToken});
});
...
usersRouter.post(
  "/update-profile", authenticate, validate(validateUpdateProfile),
...
usersRouter.put("/wishlist", authenticate, async (req: Request, res: Response) => {
  ...
  if (!Array.isArray(wishlist)) {
    res.status(400).json({error: "wishlist must be an array of productId strings."}); return;
  }
  const sanitized = wishlist.filter((id) => typeof id === "string" && id.length > 0).slice(0, 500);
  try {
    await db.doc(`users/${uid}`).set(
      {wishlist: sanitized, updatedAt: FieldValue.serverTimestamp()}, {merge: true}
    );
...
usersRouter.put("/cart", authenticate, async (req: Request, res: Response) => {
  ...
  if (!Array.isArray(cart)) {
    res.status(400).json({error: "cart must be an array."}); return;
  }
  ...
  try {
    await db.doc(`users/${uid}`).set(
      {cart: sanitized, updatedAt: FieldValue.serverTimestamp()}, {merge: true}
    );
...
usersRouter.post(
  "/set-admin", authenticate, requireAdmin, validate(validateSetAdminClaim),
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      logger.info(`[POST /users/set-admin] isAdmin=${isAdmin} set for ${targetUid} by ${req.user!.uid}`);
      res.json({success: true});
    } catch (err) {
...
usersRouter.get("/all", authenticate, requireAdmin, async (_req: Request, res: Response) => {
  try {
...
usersRouter.patch(
  "/status", authenticate, requireAdmin, validate(validateBulkStatusUpdate),
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      for (const uid of uids) {
        batch.update(db.doc(`users/${uid}`), {isActive, updatedAt: FieldValue.serverTimestamp()});
      }
      await batch.commit();
      logger.info("[PATCH /users/status] Bulk status update", {count: uids.length, isActive, by: requestingUid});

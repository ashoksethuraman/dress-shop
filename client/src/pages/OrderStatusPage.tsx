      ...
      {items?.map((it: any, idx: number) => (
        <div key={it.productId ?? idx} className={`flex items-center gap-4 px-6 ${compact ? 'py-3.5' : 'py-4'}`}>
          <div className={`rounded-xl flex items-center justify-center flex-shrink-0 ${
            compact ? 'w-12 h-12 bg-gray-100' : 'w-14 h-14 bg-indigo-50 border border-indigo-100'
          }`}>
            <FiPackage size={compact ? 16 : 18} className={compact ? 'text-gray-300' : 'text-indigo-300'} />
          </div>
      ...
      <div className="ml-auto max-w-xs space-y-1.5 text-sm">
        ...
        <div className="flex justify-between font-extrabold text-gray-900 text-base pt-2 border-t border-gray-200">
          {isFailure ? (
            ...
            <><span>Grand Total</span><span className="text-indigo-600">₹{Number(order.totalAmount).toFixed(2)}</span></>
          )}
        </div>
      </div>
    </div>
...
function buildPdf({ order, isFailure, reason, paymentId, paymentMethod, placedOn, shipAddr, billAddr, addrSame, subtotal, errorCode }: PdfCtx) {
  ...
  isFailure ? doc.setFillColor(220, 38, 38) : doc.setFillColor(79, 70, 229);
  doc.rect(0, 0, W, 30, 'F');
  sf(20, 'bold'); sc(255, 255, 255);
  doc.text('Dress Shop', L, 14);
  sf(10);
  doc.text(isFailure ? 'Payment Failure Report' : 'Order Receipt', L, 22);
  ...
  sf(12, 'bold'); isFailure ? sc(220, 38, 38) : sc(79, 70, 229);
  ...
  doc.text(
    isFailure ? 'Payment was NOT taken.  |  support@dressshop.com' : 'Thank you for shopping with Dress Shop!  |  support@dressshop.com',
    W / 2, y, { align: 'center' },
  );
  doc.save(isFailure ? `DressShop-PaymentFailed-${order.id}.pdf` : `DressShop-Receipt-${order.id}.pdf`);
}
...
export default function OrderStatusPage() {
  ...
  const downloadPdf = useCallback(() => {
    if (!order) return;
    buildPdf({ order, isFailure, reason, paymentId, paymentMethod, placedOn, shipAddr, billAddr, addrSame, subtotal, errorCode });
  ...
  if (!order) return (
    <div className="max-w-lg mx-auto px-4 py-28 text-center">
      ...
      <Link to="/" className="px-6 py-3 rounded-xl bg-indigo-600 text-white font-bold text-sm hover:bg-indigo-700 transition-colors">Back to Home</Link>
    </div>
  );
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-8">
            <button onClick={() => nav('/checkout')} className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm transition-colors shadow-md">
          ...
          <p className="text-xs text-gray-400 text-center pb-10">
            ...
            <a href="mailto:support@dressshop.com" className="text-indigo-500 hover:underline">support@dressshop.com</a>
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <nav className="text-xs text-gray-400 mb-5 flex items-center gap-1.5">
          ...
          <span>›</span><span className="text-indigo-600 font-medium">Order Confirmed</span>
        </nav>
        ...
        <div className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          ...
          <button onClick={downloadPdf} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-indigo-300 bg-white text-indigo-600 font-semibold text-sm hover:bg-indigo-50 transition-colors shadow-sm flex-shrink-0">
            <FiDownload size={14} /> Download Receipt
          </button>
        </div>

        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
          <MetaCard icon={<FiHash size={14}/>}       label="Order ID" value={order.id}                                                          mono accent="text-indigo-400" />
          <MetaCard icon={<FiCalendar size={14}/>}   label="Date"     value={placedOn}                                                               accent="text-indigo-400" />
          <MetaCard icon={<FiMail size={14}/>}       label="Email"    value={order.contactEmail ?? '—'}                                              accent="text-indigo-400" />
          <MetaCard icon={<FiCreditCard size={14}/>} label="Payment"  value={paymentId ? `···${paymentId.slice(-6)}` : (paymentMethod ?? 'Paid')}    accent="text-indigo-400" />
        </div>
        ...
        <div className={`grid ${addrSame ? '' : 'sm:grid-cols-2'} gap-5 mb-5`}>
          <SectionCard title={addrSame ? 'Billing & Shipping Address' : 'Shipping Address'}>
            <div className="flex items-start gap-3">
              <FiMapPin size={15} className="text-indigo-400 mt-0.5 flex-shrink-0" />
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-5">
          <div className="flex items-center justify-between px-6 py-4 bg-indigo-50 border-b border-indigo-100">
            ...
            <button onClick={() => nav('/shipping')} className="flex items-center gap-1 text-xs font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap pb-10">
          ...
          <button onClick={downloadPdf} className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm transition-colors shadow-md">
